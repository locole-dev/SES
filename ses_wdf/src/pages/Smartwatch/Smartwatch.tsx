...
const Smartwatch = () => {
  ...
  const [filteredData, setFilteredData] = useState<SmartWatch[]>([]);
  // setup pagination
  ...
  const handleDelete = async () => {
    ...
    const result = await apiService({ method: "DELETE", data: data, url: "/saveSmartWatch" });
  ...
  const onInsert = (e: React.FormEvent) => {
    e.preventDefault();
    navigate('/pages/smartwatch/form');
  };

  const onUpdate = (e: React.FormEvent, id: number) => {
    e.preventDefault();
    navigate(`/pages/smartwatch/form/${id}`);
  };
  ...
  return (
    <>
      <Breadcrumb pageName="Smartwatch" />
