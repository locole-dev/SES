...
const SignIn: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const passwordEncode = await Utils.generateSha1Base64(password);
    const data = { "username": username, "password": passwordEncode }
    const result = await apiService({ method: "POST", url: `/public/login`, data: data });
    if (result) {
      sessionStorage.setItem("wdf-token", result.accessToken);
      navigate('/pages/carriers');
    }
  ...
  return (
    <div className='relative flex flex-1 flex-col overflow-y-auto overflow-x-hidden justify-center h-screen	px-20'>
