...
const CarrierInsert = () => {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        const result = await apiService({ method: id ? "PUT" : "POST", url: "/saveCarrier", data: data });
    ...
    useEffect(() => {
        if (id) {
            const fetchCarrier = async () => {
                const result = await apiService({ url: `/getCarrier/${id}` });
                if (result) {
                    setName(result.name);
